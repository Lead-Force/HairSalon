 ...
 <div class="main">
  <center>
    <!--Code starts here-->
        ...
        <p>
	<form  name="HairDresserLookupForm" action="http://localhost:8080/TheHairSalon/Source Packages/Servlets/HdManagerServlet" method="post">
            <%
		...
		<div class="container-table100">
			<div class="wrap-table100">
                ...
                <% for(int row=0; row < listSize; row++) { %>
						...
						<tbody>
							<tr>
								<td class="column1"><%= b[row].getHairDresserId() %></td>
								<td class="column2"><%= b[row].getHairDresserFname() %></td>
								<td class="column3"><%= b[row].getHairDresserLname() %></td>
								<td class="column4"><%= b[row].getHairDresserAddress() %></td>
								<td class="column5"><%= b[row].getHairDresserPhone() %></td>
								<td class="column6"><%= b[row].getHairDresserEmail() %></td>
								<td class="column7"><%= b[row].isFullTime() %></td>
								<td class="column8"><%= b[row].getTimeOff() %></td>
								<td class="column9"><%= b[row].getHoursRequested() %></td>
								<td class="column10"><%= b[row].getHoursGiven() %></td>
								<td class="column11"><%= b[row].getPayRate() %></td>
							</tr>
								<!-- end loop here-->
								<% } %>
						</tbody>
	...
	<p>Insert New Hair Dresser Information:</p>
		<form name="HairDresserInsertForm" action="http://localhost:8080/TheHairSalon/HdManagerServlet" method="post">
						<table>
							<thead>
								<tr class="table100-head">
									<th class="column1">ID:</th>
	...
	<br>
        <form name="HairDresserLookupForm" action="http://localhost:8080/TheHairSalon/HdManagerServlet" method="post">
						<table>
							<thead>
								<tr class="table100-head">
